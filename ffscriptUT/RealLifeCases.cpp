...
//RealLifeCases
TEST(RealLifeCases, IntegerReverse0)
{
...
TEST(RealLifeCases, LambdaFunction)
{
	CompilerSuite compiler;
	compiler.initialize(1024);
	GlobalScopeRef rootScope = compiler.getGlobalScope();
	...
	const wchar_t* scriptCode =
		L"int doSomething(function<int(int)> callBack) {"
		L"	int param = 0;"
		L"	return callBack(param);"
		L"}"
		L"int main() {"
		L"	int compareValue = 0;"
		L"	callBack = [&compareValue](int callBackValue) -> int {"
		L"		return (int)compareValue == callBackValue;"
		L"	};"
		L"	return doSomething(callBack);"
		L"}"
		;
	...
	int res = *(int*)scriptRunner.getTaskResult();

	EXPECT_EQ(1, res);
}

TEST(RealLifeCases, LambdaFunctionRef)
{
	CompilerSuite compiler;
	compiler.initialize(1024);
	GlobalScopeRef rootScope = compiler.getGlobalScope();
