...
void ObjectHierarchyDlg::onItemCheckedChanged(HTREEITEM hItem, BOOL newState) {
	...
	FFDrawObj* attachObj = (FFDrawObj*)item.lParam;
	if (_onCheckChanged) {
		_onCheckChanged(attachObj, newState);
	}
}

void insertItem(HWND hTreeView, HTREEITEM parent, FFDrawObj* object) {
...
void ObjectHierarchyDlg::setObjectRoot(FFDrawObjRef object) {
	_rootObject = object;
}
