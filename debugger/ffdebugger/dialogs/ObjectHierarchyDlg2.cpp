	...
	case WM_NOTIFY:
		...
		if (nmhdr->code == LVN_ITEMCHANGED && (((LPNMLISTVIEW)nmhdr)->uChanged & LVIF_STATE)) {
			...
			if (oldCheckState != newCheckState) {
				onItemCheckedChanged(lpnmlv, (FFDrawObj*)lpnmlv->lParam, (newCheckState >> 12) - 1);
			}
		}
		break;
	case WM_SIZE:
...
//}
//
void ObjectHierarchyDlg2::onItemCheckedChanged(LPNMLISTVIEW lpItem, FFDrawObj* obj, BOOL newState) {
	if (_onCheckChanged) {		
		...
		while (iPos != -1) {
			if (iItem != iPos) {
				...
				lvi.iItem = iPos;
				ListView_GetItem(_hListView, &lvi);
				_onCheckChanged( ((FFDrawObj*)lvi.lParam), newState);
			}
...
void insertItem(HWND hTreeView, FFDrawObj* object, int indent) {
	...
	auto pContainer = dynamic_cast<FFContainer*>(object);
...
void ObjectHierarchyDlg2::setObjectRoot(FFDrawObjRef object) {
	_rootObject = object;
}
